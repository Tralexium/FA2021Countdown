...
func _process(delta: float) -> void:
	if mix_strength != old_mix_strength:
		...
		ground_bottom_color   = cur_set.get_color(2).linear_interpolate(next_set.get_color(2), mix_strength)
		ground_horizon_color  = cur_set.get_color(3).linear_interpolate(next_set.get_color(3), mix_strength)
